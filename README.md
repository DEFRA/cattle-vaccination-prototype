...
```bash
cd cattle-vaccination-prototype
nvm use
```
...
> [!WARNING]
> The `cattle-vaccination-prototype` is not a production ready application, it is a tool for prototyping. It is not
...
The refreshed GOV.UK brand is available and turned on by default in the `cattle-vaccination-prototype`. To turn it
...
```bash
docker build --target development --no-cache --tag cattle-vaccination-prototype:development .
```

Run:

```bash
docker run -e PORT=3000 -p 3000:3000 cattle-vaccination-prototype:development
```
...
```bash
docker build --no-cache --tag cattle-vaccination-prototype .
```
...
```bash
docker run -e PASSWORD=beepBoopBeep -e PORT=3000 -p 3000:3000 cattle-vaccination-prototype
```
...
```bash
docker run -it --entrypoint /bin/ash cattle-vaccination-prototype
```
